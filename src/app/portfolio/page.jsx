...
const Portfoliopage = () => {
  ...
  return (
    <motion.div className="h-full"
          ...
          <motion.div style={{ x }} className="flex ">
            ...
            {items.map(item => (<div className={`h-screen w-screen flex items-center justify-center bg-gradient-to-r ${item.color}`} key={item.id}>
              <div className="flex flex-col gap-8 text-white">
                ...
                <Link href={item.link} className="flex justify-end"><button className="p-2 text-sm md:p-4 md:text-md lg:p-8 lg:text-lg bg-white text-gray-600 font-semibold rounded m-4">See Demo</button></Link>
              </div>
            </div>
            ))}
          </motion.div>
      ...
      <div className="w-screen h-screen flex flex-col gap-16 items-center justify-center text-center bg-gradient-to-t from-yellow to-purple">
